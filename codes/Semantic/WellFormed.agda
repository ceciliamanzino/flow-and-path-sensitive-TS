...
data wellFStm : StmS → Storeₜ → 𝒜 → Set where
  SkipWF : {mₜ : Storeₜ} {A : 𝒜}
    → wellFormed mₜ A
    → wellFStm Skip mₜ A
    
  AssignWF : {v : Fin n} {e : ExpS} {mₜ : Storeₜ} {A : 𝒜}
    → wellFormed mₜ A
    → wellFStm (v := e) mₜ A
    
  AssignBrWF : {v : Fin n} {e : ExpS} {mₜ : Storeₜ} {A : 𝒜}  
    → wellFormed mₜ (proj₂ (transStm ⟦ v := e ⟧ A))
    → wellFStm ⟦ v := e ⟧ mₜ A
    
  SeqWF : {s₁ s₂ : StmS} {mₜ : Storeₜ} {A : 𝒜}
    → wellFStm s₁ mₜ A
    → wellFStm s₂ mₜ (proj₂ (transStm s₁ A))
    → wellFStm (Seq s₁ s₂) mₜ A
    
  IfWF : {e : ExpS} {sT sF : StmS} {mₜ : Storeₜ} {A : 𝒜}
    → wellFormed mₜ (proj₂ (transStm (If e sT sF) A))
    → wellFStm sT mₜ A
    → wellFStm sF mₜ A
    → wellFStm (If e sT sF) mₜ A
    
  WhileWF : {e : ExpS} {s : StmS} {mₜ : Storeₜ} {A : 𝒜}
...
wellFTrans : {s : Stm} {mₜ mₜ' : Storeₜ} {A : 𝒜} 
  → wellFormed mₜ A 
  → ⟨ s , mₜ ⟩⇓ₜ mₜ'
  → wellFormed mₜ' A
...
wellF-tranStm : {s : StmS} {sₜ : Stm} {mₜ mₜ' : Storeₜ} {A : 𝒜}
  → wellFStm s mₜ A
  → ⟨ sₜ , mₜ ⟩⇓ₜ mₜ'
  → wellFStm s mₜ' A
