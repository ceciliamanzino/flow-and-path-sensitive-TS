...
postulate 𝒜memEqPostVar : {currVar : ℕ} {varName : Fin n} {cV<n : currVar <ₙ n} {A A' : 𝒜} {mₜ mₜ' : Storeₜ}  → ⟨ 𝒜assignAux currVar cV<n A' A , mₜ ⟩⇓ₜ mₜ'  → currVar <ₙ toℕ varName  → lookup mₜ varName ≡ lookup mₜ' varName
...
-- for that variable has been executed. 
postulate 𝒜memEqPreVar : {currVar : ℕ} {varName : Fin n} {cV<n : currVar <ₙ n} {A A' : 𝒜} {mₜ mₜ' : Storeₜ}  → ⟨ 𝒜assignAux currVar cV<n A' A , mₜ ⟩⇓ₜ mₜ' → toℕ varName ≤ₙ currVar   → wellFormed mₜ A'  → lookupₜ mₜ A varName ≡ lookupₜ mₜ' A' varName
