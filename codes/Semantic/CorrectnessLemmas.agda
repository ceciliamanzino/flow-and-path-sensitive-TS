...
expEquality : {e : ExpS} {m : Storeᵢ} {mₜ : Storeₜ} {v v' : ℕ} {A : 𝒜}
  → m ==ₘ mₜ - A
  → ⟦ e ⟧ₑ m ≡ v 
  → ⟦ transExp e A ⟧ₜ mₜ ≡ v' 
  → v ≡ v' 
...
-- auxiliary lemmas
toℕ-cancel-< : ∀ {n} {i j : Fin n} → toℕ i <ₙ toℕ j → i < j
toℕ-cancel-< i<j = i<j

m<n⇒m<1+n : ∀ {m n : ℕ} → m <ₙ n → m <ₙ suc n
m<n⇒m<1+n {m} {n}  m<n = s≤s (<⇒≤ m<n)

m<1+n⇒m≤n : ∀ {m n : ℕ} → m <ₙ suc n → m ≤ₙ n
m<1+n⇒m≤n (s≤s p) = p
...
𝒜memEqPostVar : {currVar : ℕ} {x : Fin n} {cV<n : currVar <ₙ n} {A A' : 𝒜} {mₜ mₜ' : Storeₜ}
                → ⟨ 𝒜assignAux currVar cV<n A' A , mₜ ⟩⇓ₜ mₜ'
                → currVar <ₙ toℕ x
                → lookup mₜ x ≡ lookup mₜ' x
𝒜memEqPostVar {zero} {_} {z<n} {A} {A'} _ _ with lookup A' (fromℕ< z<n) ≟ₙ lookup A (fromℕ< z<n)
𝒜memEqPostVar {zero} Skipₜ _                                  | yes _ = refl
𝒜memEqPostVar {zero} {x} {z<n} {_} {_} {mₜ} Assignₜ z<vN | no _ =
 let toNz=0 : toℕ (fromℕ< z<n) ≡ 0
     toNz=0 = toℕ-fromℕ< z<n
     toNz<toNvN : toℕ (fromℕ< z<n) <ₙ toℕ x
     toNz<toNvN = subst (λ y → y <ₙ toℕ x) (sym toNz=0) z<vN
     z<vN : (fromℕ< z<n) < x
     z<vN = toℕ-cancel-< toNz<toNvN 
     vN<>z : x ≢  (fromℕ< z<n)
     vN<>z = ≢-sym (<⇒≢  z<vN)
 in sym (lookupy∘changex (fromℕ< z<n) x mₜ vN<>z)
𝒜memEqPostVar {suc currVar'} {_} {cV<n} {A} {A'} _ _ with lookup A' (fromℕ< cV<n) ≟ₙ lookup A (fromℕ< cV<n)
𝒜memEqPostVar {suc _} {x} (Seqₜ Skipₜ d) cV<vN    | yes _ = 𝒜memEqPostVar d (<-pred (<-trans cV<vN (n<1+n (toℕ x)))) 
𝒜memEqPostVar {suc currVar'} {x} {cV<n} {_} {_} {mₜ} {mₜ'} (Seqₜ {.mₜ} {mₜ1} {.mₜ'} Assignₜ d) cV<vN | no _ = 
    let toNcV=cV : toℕ (fromℕ< cV<n) ≡ suc currVar'
        toNcV=cV = toℕ-fromℕ< cV<n
        toNcV<toNvN : toℕ (fromℕ< cV<n) <ₙ toℕ x
        toNcV<toNvN = subst (λ y → y <ₙ toℕ x) (sym toNcV=cV) cV<vN
        fNcV<vN : (fromℕ< cV<n) < x
        fNcV<vN = toℕ-cancel-< toNcV<toNvN 
        vN<>fNcV : x ≢  (fromℕ< cV<n)
        vN<>fNcV = ≢-sym (<⇒≢  fNcV<vN)
        lmₜvN=lmₜ1vN : lookup mₜ x ≡ lookup mₜ1 x
        lmₜvN=lmₜ1vN = sym (lookupy∘changex (fromℕ< cV<n) x mₜ vN<>fNcV)
        lmₜ1vN=lmₜ'vN : lookup mₜ1 x ≡ lookup mₜ' x
...
𝒜memEqPreVar {zero} {zero} {A' = A'} {mₜ = mₜ} Assignₜ _ wFmₜA' | no _ = sym (lookupₜx∘changeₜx {n} {_} {lookup A' zero} zero mₜ (wFmₜA' zero))
𝒜memEqPreVar currVar@{suc _} {x} {cV<n} {A} {A'} _ _ _ with toℕ x ≟ₙ currVar | lookup A' (fromℕ< cV<n) ≟ₙ lookup A (fromℕ< cV<n)
𝒜memEqPreVar {suc currVar'} {x} {cV<n} {A} {A'} {mₜ} {mₜ'} (Seqₜ Skipₜ d) _ _ | yes vN=cV | yes lA'cV=lAcV = 
  let lmₜvN=lmₜ'vN : lookup mₜ x ≡ lookup mₜ' x
      ...
      lA'vN=lAvN : lookup A' x ≡ lookup A x
      lA'vN=lAvN = subst (λ x → lookup A' x ≡ lookup A x) (sym vN=cV) lA'cV=lAcV
  in cong₂ (λ x y → lookupOr0 x y) (sym lA'vN=lAvN) lmₜvN=lmₜ'vN
𝒜memEqPreVar {suc currVar'} {x} {cV<n} {A} {A'} {mₜ} {mₜ'} (Seqₜ {.mₜ} {mₜ1} {.mₜ'} Assignₜ d) _ wFmₜA' | yes vN=cV | no _ = 
  let finCurrVar = fromℕ< cV<n
      ...
      lmₜAvN=lmₜ1A'vN : lookupₜ mₜ A x ≡ lookupₜ mₜ1 A' x
      lmₜAvN=lmₜ1A'vN = subst (λ x → lookupₜ mₜ A x ≡ lookupₜ mₜ1 A' x) cV=vN (sym lmₜ1A'cV=lmₜAcV)
  in subst (λ y → lookupₜ mₜ A x ≡ lookupOr0 (lookup A' x) y) lmₜ1vN=lmₜ'vN lmₜAvN=lmₜ1A'vN
𝒜memEqPreVar {suc _} (Seqₜ Skipₜ d) vN≤cV wFmₜA' | no vN<>cV | yes _ = 𝒜memEqPreVar d (m<1+n⇒m≤n (≤∧≢⇒< vN≤cV vN<>cV)) wFmₜA'
𝒜memEqPreVar currVar@{suc _} {x} {cV<n} {A} {A'} {mₜ} {mₜ'} (Seqₜ {.mₜ} {mₜ1} {.mₜ'} assign@Assignₜ d) vN≤cV wFmₜA' | no vN<>cV | no _ =
  let vN<>fromN<cV : x ≢  fromℕ< cV<n
      vN<>fromN<cV = (λ vN=fromN<cV → vN<>cV (subst (λ x → toℕ x ≡ currVar) (sym vN=fromN<cV) (toℕ-fromℕ< cV<n))) 
      lmₜ1AvN=lmₜAvN : lookupₜ mₜ1 A x ≡ lookupₜ mₜ A x
      lmₜ1AvN=lmₜAvN = lookupₜy∘changeₜx (fromℕ< cV<n) x mₜ vN<>fromN<cV
      lmₜ1AvN=lmₜ'A'vN : lookupₜ mₜ1 A x ≡ lookupₜ mₜ' A' x
      lmₜ1AvN=lmₜ'A'vN = 𝒜memEqPreVar d (m<1+n⇒m≤n (≤∧≢⇒< vN≤cV vN<>cV)) (wellFTrans {A = A'} wFmₜA' assign)
...
:=𝒜-memEq : {A A' : 𝒜} {mₜ mₜ' : Storeₜ} 
  → ⟨ A' :=𝒜 A , mₜ ⟩⇓ₜ mₜ'
  → wellFormed mₜ A'
  → ∀ x → lookupₜ mₜ A x ≡ lookupₜ mₜ' A' x   -- mₜ - A ==ₘₜ mₜ' - A'
:=𝒜-memEq d wFmₜA' x with n ≟ₙ zero 
...                           | no n<>0 = let n' , n'+1=n = 0<n=>n'+1=n (n≢0⇒n>0 n<>0)
                                              n-1=n' : pred n ≡ pred (suc n')
                                              n-1=n' = (cong pred (sym n'+1=n))
                                              vN≤n-1 : toℕ x ≤ₙ pred n
                                              vN≤n-1 = (toℕ≤pred[n] x)
                                          in 𝒜memEqPreVar d (subst (λ y → toℕ x ≤ₙ y) n-1=n' vN≤n-1) wFmₜA'
